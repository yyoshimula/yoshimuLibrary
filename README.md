...
> 📖 **全関数のリファレンスは [`MANUAL.md`](MANUAL.md) を参照**（ソースコードと突き合わせて生成した約300関数の入出力仕様、および末尾にコードとドキュメントコメントの不整合一覧）。
...
> 📖 **See [`MANUAL.md`](MANUAL.md) for the full function reference** — I/O specs for ~300 functions verified against the source, plus a code/doc-comment inconsistency list at the end.
